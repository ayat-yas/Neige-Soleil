        ...
        <div class="card-body">
            <form method="POST" action="index.php?page=vue_insert_reservation">
                ...
                <div class="form-grid">
                        ...
                        <select id="idgite" name="idgite" required>
                            ...
                            <?php foreach ($gites_list as $g): ?>
                                <option value="<?php echo $g['idgite']; ?>"
                                    ...
                                    <?php echo htmlspecialchars($g['adresse'] . ' — ' . number_format($g['loyer'], 0, ',', ' ') . ' €/j'); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <!-- Statut et rapport réservés à l'admin -->
                    <?php if ($estProprio): ?>
                    <div class="form-group">
                        <label for="statut_r">Statut *</label>
                        <select id="statut_r" name="statut_r" required>
                            <?php
                            $statuts = ['à valider', 'en cours', 'terminé', 'non réservé'];
                            foreach ($statuts as $s):
                                $sel = (isset($donnee['statut_r']) && $donnee['statut_r'] === $s)
                                    || (!isset($donnee['statut_r']) && $s === 'à valider');
                            ?>
                                <option value="<?php echo $s; ?>" <?php echo $sel ? 'selected' : ''; ?>>
                                    <?php echo ucfirst($s); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="form-group full">
                        ...
                        <textarea id="rapport" name="rapport" rows="3"
                                  placeholder="Observations, incidents…"><?php echo htmlspecialchars($donnee['rapport'] ?? ''); ?></textarea>
                    </div>
                    <?php else: ?>
