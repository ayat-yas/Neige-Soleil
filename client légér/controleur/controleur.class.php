...
class Controleur {
    ...
    private function gererConnexion(): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!in_array($type_user, ['client', 'proprio'], true)) {
                ...
                // Supporte les mdp hashés (nouveaux) et en clair (données de test)
                $mdp_ok = $user && (password_verify($mdp, $user['mdp']) || $mdp === $user['mdp']);
    ...
    private function gererCrud(string $table): bool {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST'
            ...
            $message = $ok ? ucfirst($table) . ' supprimé avec succès.' : 'Erreur lors de la suppression.';
        }
        ...
        if ($table === 'reservation') {
            ...
            $this->vue('vue_select_' . $table, ['donnees' => $donnees, 'message' => $message, 'table' => $table]);
        }
        return true;
    }
    ...
    private function gererFormulaire(string $table): void {
        ...
        // Un client peut créer une réservation, mais pas modifier les autres entités
        $client_cree_resa = ($table === 'reservation' && !$idValue && !$this->estProprio());
        if (!$client_cree_resa) $this->autoriserProprio();

        if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['action'])) {
            $data = $_POST;
            unset($data['action']);

            // Normalise idcategorie (null si non sélectionné)
            if ($table === 'client') {
                $data['idcategorie'] = !empty($data['idcategorie']) ? (int)$data['idcategorie'] : null;
            }
            ...
            try {
                if ($_POST['action'] === 'insert') {
                    if ($table === 'reservation') {
                        if ($this->estProprio()) {
                            // L'admin crée pour un client → en attente de validation
                            $data['statut_r'] = 'à valider';
                        } else {
                            // Le client réserve lui-même → directement en cours
                            $data['idclient'] = $_SESSION['user_id'];
                            $data['statut_r'] = 'en cours';
                        }
                    }
                    $method = 'insert' . ucfirst($table);
                    if (!method_exists($this->modele, $method)) throw new Exception("Méthode $method introuvable.");
        ...
        if ($idValue) {
            ...
            if (!$donnee) $message = 'Enregistrement introuvable.';
        }

        // Données supplémentaires pour les listes FK
        $extra = [];
        if ($table === 'gite') {
            ...
            $extra['categories'] = $this->modele->selectAll('categorie');
        }

        $this->vue('vue_insert_' . $table, array_merge(['donnee' => $donnee, 'message' => $message, 'idName' => $idName], $extra));
    }
    ...
    protected function vue(string $nom, array $donnees = []): void {
        extract($donnees);
        $chemin = 'vue/' . $nom . '.php';
        file_exists($chemin) ? require $chemin : require 'vue/erreur.php';
    }
    ...
    private function autoriserProprio(): bool {
        if (!$this->estProprio()) $this->redirect('home');
        return true;
    }

    private function autoriserConnecte(): bool {
        if (!isset($_SESSION['user_id'])) $this->redirect('connexion');
        return true;
    }
}
