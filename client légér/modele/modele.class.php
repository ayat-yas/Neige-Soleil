...
class Modele {
    ...
    public function getUtilisateurByEmail(string $email, string $table): array|false {
        // Whitelist stricte pour éviter toute injection via le nom de table
        if (!in_array($table, ['client', 'proprio'], true)) return false;
    ...
    public function selectAll(string $table): array {
        $autorisees = ['client', 'proprio', 'gite', 'reservation', 'categorie'];
        if (!in_array($table, $autorisees, true)) return [];
    ...
    public function delete(string $table, string $idName, mixed $idValue): bool {
        if (!in_array($table, ['client', 'proprio', 'gite', 'reservation'], true)) return false;
    ...
    public function insertClient(array $data): bool {
        $stmt = $this->pdo->prepare(
            "INSERT INTO client (nom, prenom, adresse, email, mdp, telephone, idcategorie)
             VALUES (:nom, :prenom, :adresse, :email, :mdp, :telephone, :idcategorie)"
        );
        return $stmt->execute([
            'nom'         => $data['nom'],
            'prenom'      => $data['prenom'],
            ...
            'idcategorie' => !empty($data['idcategorie']) ? (int)$data['idcategorie'] : null,
        ]);
    }
    ...
    public function updateClient(array $data): bool {
        ...
        $stmt = $this->pdo->prepare(
            "UPDATE client SET nom=:nom, prenom=:prenom, adresse=:adresse,
             email=:email, telephone=:telephone, idcategorie=:idcategorie $mdp_sql
    ...
    // JOIN client + gite avec filtres optionnels
    ...
    public function compterTable(string $table): int {
        if (!in_array($table, ['client', 'proprio', 'gite', 'reservation'], true)) return 0;
    ...
    public function reservationsRecentes(int $limit = 5): array {
        $stmt = $this->pdo->prepare(
            "SELECT r.idreservation, r.datedebut, r.datefin, r.prix, r.statut_r,
             ...
             INNER JOIN client c ON r.idclient = c.idclient
             INNER JOIN gite   g ON r.idgite   = g.idgite
             ORDER BY r.idreservation DESC LIMIT :lim"
        );
